...
**שפה — עברית טבעית ושיחתית (קריטי):**
- כתוב כמו שמסבירים ללקוח בטלפון, בעברית זורמת ופשוטה. **בלי ניסוחים מתורגמים או מוזרים.**
- **על הוצאה כתוב "ניצלנו X בתקציב הפרסום"** (או "הוצאנו X"). לעולם אל תכתוב "הכנסנו X לעבודה" — זה לא עברית תקנית. דוגמה נכונה: "השבוע ניצלנו ₪595 בתקציב הפרסום בחשבון המודעות".
- הימנע ממטבעות-לשון מסורבלות, תרגום מילולי מאנגלית, וביטויים שאף אחד לא אומר בעברית. אם משפט נשמע מוזר בקריאה בקול — נסח אותו מחדש בפשטות.
- מונחים מקצועיים (CTR, ROAS) — רק אם באמת צריך, ותמיד עם הסבר קצר בעברית.
...
- **אסור להשתמש במקף ארוך (— או –) בשום מקום בדוח.** במקום מקף ארוך השתמש בנקודתיים, פסיק או נקודה. (מקף רגיל קצר בתאריכים/שמות מותר.)
...
/**
 * מסיר מקפים ארוכים (— em, – en) מהדוח לחלוטין — רשת ביטחון מעל הוראת ה-prompt.
 * מקף ארוך עם רווחים → פסיק; מקף ארוך צמוד → מוסר. מקף רגיל קצר (-) לא נוגעים בו.
 */
export function stripLongDashes(text: string): string {
  return text
    .replace(/(\d)\s*[—–]\s*(\d)/g, "$1-$2") // טווח מספרים/תאריכים (9–15) → מקף רגיל קצר
    .replace(/ *[—–] +/g, ", ")      // מקף ארוך עם רווחים → פסיק (טבעי בפרוזה)
    .replace(/ +[—–] */g, ", ")      // גיבוי לכיוון ההפוך
    .replace(/[—–]/g, " ")            // מקף ארוך צמוד שנותר → רווח
    .replace(/ {2,}/g, " ")           // איחוד רווחים כפולים
    .replace(/ ([.,:;!?)])/g, "$1")   // ניקוי רווח לפני פיסוק
    .replace(/,\s*,/g, ",");          // ניקוי פסיקים כפולים
}

/** בונה בלוק "הנחיות קבע של הלקוח" בולט למערכת — גובר על מבנה ברירת המחדל */
export function clientRulesBlock(instructions: string): string {
  const t = instructions.trim();
  if (!t) return "";
  return `\n\n---\n\n**הנחיות קבע של הלקוח הזה (חובה לכבד — הן גוברות על מבנה ברירת המחדל של הדוח. אם הלקוח ביקש להשמיט פלטפורמה/מוצר/סקשן — אל תכלול אותו כלל, גם אם יש נתונים):**\n${t}`;
}
...
export async function generateWeeklyReportContent(
  ...
  const response = await anthropic.messages.create({
    model: MODEL,
    max_tokens: 3000,
    system: `${systemBase}\n\n---\n\n${businessKnowledge}\n\n---\n\n${REPORT_INSTRUCTIONS}${clientRulesBlock(instructions)}`,
    messages: [{ role: "user", content: userMessage }],
  });

  const block = response.content.find((b) => b.type === "text");
  return block && "text" in block ? stripLongDashes(block.text.trim()) : "";
}
